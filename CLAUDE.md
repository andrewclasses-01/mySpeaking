...
- **Bố cục GHIM**: desktop ≥1024px khoá `100dvh` — video + form đứng yên, CHỈ "Mistakes found" cuộn (CSS riêng `#appScreen:not(.hidden)`; KHÔNG dùng `lg:flex` cho phần tử toggle `.hidden` — bẫy cascade). Mobile: header cuộn qua, CỤM VIDEO sticky top-0. 2 cột desktop BẰNG chiều ngang (`grid-cols-2`), video cân cao form (~522/521).
  ...
  - **Chặng 29 (21/07/2026): 2 khối trắng video/check lỗi LUÔN CAO BẰNG NHAU** mọi cỡ/tỷ lệ cửa sổ desktop: cột video + thẻ video = flex dọc giãn hết cột; `.video-shell` ở ≥1024px **bỏ aspect-ratio 16:9 cứng** (`flex:1`, video tự viền đen như YouTube khi lệch tỷ lệ); videoCtrl/stopwatchWrap/videoStatus `lg:shrink-0`. Mobile giữ 16:9 + sticky.
